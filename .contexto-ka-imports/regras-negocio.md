...
- Espelho Yury Motoboy/Minas (`yury_inventory_balances`): só leitura; na aba Estoque há **Estoque Loja**, **Estoque Motoboy** e **Estoque Minas**. Motoboy/Minas não têm entrada/saída/reenvio. Lista mostra a foto do catálogo (mesmo thumbnail do saldo da loja) casando `productId` ou o nome. `productId` da Yury; quantity 0 permanece; produto só num pool zera o outro; sumir dos dois arrays zera os dois. Webhook grava `balances`, não o delta. Não soma os pools e não baixa pedido KA aí.
