...
- Cupons: % ou valor fixo, min. pedido, max usos, ativo/inativo. Pedido mínimo do cupom no checkout/API usa subtotal + frete (sem o +10% do seguro).
- Seguro de envio (+10%): base = `max(0, subtotal − desconto do cupom)`. Sem cupom continua 10% do subtotal. A API recalcula (`computeShippingInsuranceAmount`); não grava o valor enviado pelo cliente. Edição de pedido no admin usa a mesma regra. Frete e crédito de afiliado entram depois.
