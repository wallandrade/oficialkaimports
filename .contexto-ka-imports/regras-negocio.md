...
- Motoboy no checkout: setting `motoboy_eligible_product_ids` (JSON; vazio = todos). Carrinho misto some a opção; API `400 MOTOBOY_NOT_ELIGIBLE`. Frete grátis Motoboy usa `checkout_free_shipping_min_motoboy`, separado de `checkout_free_shipping_min_subtotal`. Lookup de bairro normaliza acento, tira `(…)` e espaços; senão faixa CEP mais estreita (**cidade só desempata**; CEP na faixa já libera). Duração do slot = `interval_hours` do bairro/faixa (não mais preço ≤ 75). Cópia Motoboy (N) inclui PIX pendente, sem telefone; layout tipo Yury (`#161`, sem prefixo `KA-`, sem linha de agendamento). Lote da filial prefixa o `site_name` no título (Motoboy, Envios 48h, Outros, Compra); loja 1 sem sufixo. Procurando produto continua fora. Job de expire 24h cancela o pedido **e** apaga a reserva. Após 18h (SP) o calendário começa amanhã. Cliente vê data/hora na Minha conta e no Success. Isolamento EnvioEcom inalterado. Sem portal de preço e sem last-mile. **Cobertura (bairros/faixas)** é espelho da Yury quando `YURY_MOTOBOY_SYNC_TOKEN` está setado (pull 15 min + webhook HMAC); cadastro local de cobertura fica bloqueado. **Estoque Motoboy/Minas** é outro espelho da Yury (`yury_inventory_balances`, pull 3 min + webhook); não mistura com o estoque da loja nem baixa pedido KA nesses pools.
