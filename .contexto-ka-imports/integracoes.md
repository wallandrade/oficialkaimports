...
- Rotas admin (`hasGlobalAccess`, não seller-scoped): quote/create/labels/sync/cancel/bind-id, tracking-board, config, CRUD accounts, GET/PUT `shipment-item-name`, registrar webhook em **todas** as contas. Quote/create aceitam `accountId` no body e devolvem `accountId`. No split, quote/create/labels/sync/cancel/bind exigem `packageId` (`NEED_PACKAGE_ID`). Labels/sync/cancel no pacote usam só barcode/`shipment_id` **daquele** pacote (`bindEnvioEcomFieldsToPackage`); não caem no `trackingCode` do pedido. Pacote sem envio próprio → `SHIPMENT_ID_REQUIRED` / `NO_SHIPMENT`. Sync/labels/cancel fazem fallback entre contas. `PATCH /admin/orders/:id/tracking-code` no split também pede `packageId` (ou escolhe o pacote sem envio); grava barcode no pacote e devolve `packages[]`. Parse/match da etiqueta (`tracking-label/parse` e `/match`) devolvem `mapOrderWithPackages`.
...
- Público: `POST /api/webhook/envioecom` (2xx rápido; match **exato** barcode → `external_order_number` → `shipment_id`; tenta **`order_shipments` primeiro** (`findPackageForEnvioEcomWebhook`) e só então `orders`; `shipmentEventMatchesOrder` recusa pedido/pacote sem vínculo ou com outro envio; **não** casa prefixo `{n}-` nem UUID do pedido). Aceita body plano ou `data`/`shipment`; eventos com barcode/status não são ignorados se o pedido ainda estiver ligado àquele envio. Idempotente barcode+status. Histórico vem de `status_history` (status + `location` cidade/unidade, também `location.name`, `city_name`, `municipio`); 2+ eventos substituem o JSON local, 1 evento faz append idempotente. Não grava nota “Status atualizado ao consultar rastreio”. Status cancelado/cancelamento no persist **desvincula** o pedido (`detachEnvioEcomShipment`) ou o pacote (`detachEnvioEcomPackage`). Lista por CPF ignora envios com status bloqueado.
- Create (`POST /shipping/create`): sempre **1 item** e **1** entrada em `shipments[]` por chamada (nunca `[a,b]` no mesmo POST). `name` / `quantity` / `unit_cost` vêm dos settings da loja (defaults Mercadoria, 1, R$ 5). O campo `cost` da DACE (valor declarado) é qty × unit_cost, **não** o R$ 5 da cotação. Nunca nome, quantidade nem preço do pedido/catálogo. Envios já criados não mudam; para alterar é cancelar + criar de novo. Se o status local já estiver cancelado/aguardando cancelamento, o create **desvincula antes** e gera orderId novo. **orderId** 1:1: 1ª vez `{orderNumber}-{8 chars do id}`; se já houve histórico/envio, `{base}-{sufixo}`. **orderId do pacote (split):** `{n}-{8chars}-{pool}`; retry `{base}-{pool}-{sufixo}`. Até 3 tentativas em `DUPLICATE_ORDER`. Antes do create a API recusa CPF/CNPJ que não tenha 11/14 dígitos (ou só zeros), telefone com menos de 10 dígitos e e-mail sem `@`. Não envia mais CPF placeholder `000.000.000-00`. Erro da EnvioEcom é logado (`[EnvioEcom] code/message/details`) e o toast junta `message` + `details`.
- Cancel (`POST .../cancel`): chama a API EE se houver identificador; **sempre** detach depois (mesmo se a EE falhar ou ficar em Aguardando cancelamento). No 1:1 zera colunas `envioecom_*` do pedido; no split zera o pacote e faz rollup no pai. Não altera `enviado` nem estoque. Status local vira o cancelado da API ou **Aguardando cancelamento**.
- `enviado=true` via `ensureOrderMarkedEnviado` só em status de coleta/postagem/trânsito, não no PDF/DC-e. No split o pai só marca quando **todos** os pacotes já teriam `enviado`. Etiqueta pronta ainda chama `completeOrderLogistics` (vaga `shipped`). `hasEnvioEcomLabelReady` aceita **Etiqueta emitida**, **Etiqueta gerada**, **Aguardando coleta** e DC-e, além de PDF em `envioecomLabelUrl`; no split é AND de todos os pacotes. Ao gerar PDF, status “Envio criado” vira **Etiqueta emitida**. Status **Cancelado** / **Aguardando cancelamento** (webhook/sync/cancel): `hasEnvioEcomLabelReady` fica false mesmo com PDF; `isLabelBlockedStatus` impede Etiqueta EE de gerar; se `enviado` ainda é false, `allocateOrderLogistics` devolve o pedido à fila 48/72/96h. Rollup **não** copia o 1º PDF para o pai se faltar URL em algum pacote.
