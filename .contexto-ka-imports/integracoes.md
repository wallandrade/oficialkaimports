...
- `enviado=true` via `ensureOrderMarkedEnviado` só em status de coleta/postagem/trânsito (`expedido` inclusive), não no PDF/DC-e. No split o pai só marca quando **todos** os pacotes já teriam `enviado`. Etiqueta pronta ainda chama `completeOrderLogistics` (vaga `shipped`). `hasEnvioEcomLabelReady` aceita **Etiqueta emitida**, **Etiqueta gerada**, **Aguardando coleta** e DC-e, além de PDF em `envioecomLabelUrl`; no split é AND de todos os pacotes. Ao gerar PDF, status “Envio criado” vira **Etiqueta emitida**. Sem PDF/status após unlink (ou cancel): `hasEnvioEcomLabelReady` false; se `enviado` ainda é false, `allocateOrderLogistics` devolve o pedido à fila 48/72/96h. Rollup **não** copia o 1º PDF para o pai se faltar URL em algum pacote.
