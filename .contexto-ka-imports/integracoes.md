...
- **Vincular EE**: modal no card (não `window.prompt`). Admin cola ID (4–10 dígitos → `shipment_id`) ou rastreio (`barcode`). `POST /api/admin/envioecom/orders/:id/sync` com esse body busca na EE (`getById` / `getByIdentifier`; se o admin colou e falhou, tenta `list` por CPF/CEP/nome e **ignora** envios cancelados/aguardando cancelamento) e grava com `persistEnvioEcomShipment`. Não cria envio novo. Etiqueta com barcode `EC…` reabre o mesmo modal.
...
- Público: `POST /api/webhook/envioecom` (2xx rápido; match **exato** barcode → `external_order_number` → `shipment_id`; `shipmentEventMatchesOrder` recusa pedido sem vínculo ou com outro envio; **não** casa prefixo `{n}-` nem UUID do pedido). Aceita body plano ou `data`/`shipment`; eventos com barcode/status não são ignorados se o pedido ainda estiver ligado àquele envio. Idempotente barcode+status. Histórico vem de `status_history` (status + `location` cidade/unidade, também `location.name`, `city_name`, `municipio`); 2+ eventos substituem o JSON local, 1 evento faz append idempotente. Não grava nota “Status atualizado ao consultar rastreio”. Status cancelado/cancelamento no persist **desvincula** o pedido (`detachEnvioEcomShipment`). Lista por CPF ignora envios com status bloqueado.
- Create (`POST /shipping/create`): sempre **1 item**. `name` / `quantity` / `unit_cost` vêm dos settings da loja (defaults Mercadoria, 1, R$ 5). O campo `cost` da DACE (valor declarado) é qty × unit_cost, **não** o R$ 5 da cotação. Nunca nome, quantidade nem preço do pedido/catálogo. Envios já criados não mudam; para alterar é cancelar + criar de novo. Se o status local já estiver cancelado/aguardando cancelamento, o create **desvincula antes** e gera orderId novo. **orderId**: 1ª vez `{orderNumber}-{8 chars do id}`; se já houve histórico/envio, `{base}-{sufixo}`. Até 3 tentativas em `DUPLICATE_ORDER`. Antes do create a API recusa CPF/CNPJ que não tenha 11/14 dígitos (ou só zeros), telefone com menos de 10 dígitos e e-mail sem `@`. Não envia mais CPF placeholder `000.000.000-00`. Erro da EnvioEcom é logado (`[EnvioEcom] code/message/details`) e o toast junta `message` + `details`.
- Cancel (`POST .../cancel`): chama a API EE se houver identificador; **sempre** `detachEnvioEcomShipment` depois (mesmo se a EE falhar ou ficar em Aguardando cancelamento). Zera `envioecom_shipment_id`, barcode, tracking key, external orderId, label URL, deliveryMode e freight EE; limpa `trackingCode`/`trackingLabelUrl` se forem os da EE. Não altera `enviado` nem estoque. Status local vira o cancelado da API ou **Aguardando cancelamento**.
- `enviado=true` via `ensureOrderMarkedEnviado` só em status de coleta/postagem/trânsito, não no PDF/DC-e. Etiqueta pronta ainda chama `completeOrderLogistics` (vaga `shipped`). `hasEnvioEcomLabelReady` aceita **Etiqueta emitida**, **Etiqueta gerada** (texto da EnvioEcom no create/sync/webhook) e DC-e, além de PDF em `envioecomLabelUrl`. Ao gerar PDF, status “Envio criado” vira **Etiqueta emitida**. Status **Cancelado** / **Aguardando cancelamento** (webhook/sync/cancel): `hasEnvioEcomLabelReady` fica false mesmo com PDF; `isLabelBlockedStatus` impede Etiqueta EE de gerar; se `enviado` ainda é false, `allocateOrderLogistics` devolve o pedido à fila 48/72/96h.
