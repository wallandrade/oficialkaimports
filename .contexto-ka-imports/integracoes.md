...
- Cliente: `GET /api/me/orders/:id/tracking` (soft-sync de um pedido). `POST /api/me/orders/tracking-sync` atualiza até 8–10 pedidos abertos do cliente. Meus pedidos mostra a mesma timeline em linha do board admin (`ShippingStatusTimeline`) e faz poll ~2 min nos abertos.
