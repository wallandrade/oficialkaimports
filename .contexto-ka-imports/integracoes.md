...
- Cliente: `GET /api/me/orders/:id/tracking` (soft-sync de um pedido). `POST /api/me/orders/tracking-sync` atualiza até 8–10 pedidos abertos **ou com histórico sem cidade** e mescla `packages`. Meus pedidos mostra a mesma timeline em linha do board admin (`ShippingStatusTimeline`) e faz poll ~2 min nos abertos/sem cidade. Split: código/status por origem; “Entregue” só se todos entregues.
