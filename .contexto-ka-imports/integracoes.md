...
- Create (`POST .../create`): sempre **1 item** e **1** entrada em `shipments[]` por chamada (nunca `[a,b]` no mesmo POST). `name` / `quantity` / `unit_cost` vêm dos settings da loja (defaults Mercadoria, 1, R$ 5). O campo `cost` da DACE (valor declarado) é qty × unit_cost, **não** o R$ 5 da cotação. Nunca nome, quantidade nem preço do pedido/catálogo. Se ainda houver vínculo ativo (ID, barcode, PDF ou status não cancelado) → **409 `SHIPMENT_EXISTS`**. Se só restar status cancelado/aguardando, o create **desvincula antes**. **orderId** 1:1: 1ª vez `{orderNumber}-{8 chars do id}`; depois de desvincular/cancelar (`unlinked && prev` ou status cancelado) `{base}-{sufixo}`. **orderId do pacote (split):** `{n}-{8chars}-{pool}`; retry `{base}-{pool}-{sufixo}`. Até 3 tentativas em `DUPLICATE_ORDER`. Antes do create a API recusa CPF/CNPJ que não tenha 11/14 dígitos (ou só zeros), telefone com menos de 10 dígitos e e-mail sem `@`. Não envia mais CPF placeholder `000.000.000-00`. Erro da EnvioEcom é logado (`[EnvioEcom] code/message/details`) e o toast junta `message` + `details`.
