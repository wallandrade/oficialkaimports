...
- Colocar Compra/Envios/Motoboy/Outros no card **Pedidos para Enviar** do dashboard, deixar Motoboy em **Pedido normal**, ou mostrar todas as cópias em qualquer pill. A cópia de lote fica embaixo das pills e **só da aba ativa**; Motoboy tem pill própria (reenvio ativo e `is_aguardando_estoque` na frente).
...
- Realocar split apagando pacotes sem copiar `envioecom_*` da **mesma** origem, ou remover pool que ainda tem etiqueta. Use **Editar divisão**; 409 `SPLIT_HAS_LABEL` se a origem com EE sair — Desvincular antes.
